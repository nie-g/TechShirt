...
interface DesignRecord {
  ...
  status: "in_progress" | "completed" | "approved" | "pending_revision" | "in_production"| "pending_pickup";
  created_at?: number;
...
const Designs: React.FC = () => {
  ...
  "all" | "in_progress" | "finished" | "billed" | "approved" | "pending_revision"| "in_production" | "pending_pickup"
  >("all");
  ...
  // fetch designs assigned to this designer
  const designerDesigns = useQuery(
    api.designs.getDesignsByDesigner,
    user ? { designerId: user._id } : "skip"
  ) as DesignRecord[] | undefined;

  const requestIds = designerDesigns?.map((d) => d.request_id) ?? [];
  ...
  const filteredDesigns = (designerDesigns ?? [])
    .filter((d) => {
        ...
        <main className="p-4 sm:p-6 md:p-8 flex flex-col gap-6 w-full max-w-7xl mx-auto overflow-x-hidden">
          ...
          <div className="flex overflow-x-auto pb-2 hide-scrollbar">
            <div className="flex space-x-2 min-w-max">
              {["all", "in_progress","pending_revision", "completed", "pending_pickup", "in_production", "approved"].map(
                (tab) => (
                  <button
                    key={tab}
            ...
            {filteredDesigns.length === 0 ? (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Designs assigned to you will appear here
                </p>
              </div>
            ) : (
              <>
